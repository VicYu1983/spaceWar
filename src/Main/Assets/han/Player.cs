...
namespace Model
{
	public class Player : MonoBehaviour
	{
		...
		public void Shoot(){
			...
			bullet.GetComponent<Bullet> ().body.transform.localPosition = body.transform.position + new Vector3 ((float)Math.Sin(body.transform.eulerAngles.z*3.14/180)*-7, (float)Math.Cos(body.transform.eulerAngles.z*3.14/180)*7);
			bullet.GetComponent<Bullet> ().body.transform.localRotation = body.transform.localRotation;
			bullet.GetComponent<Bullet>().body.GetComponent<Rigidbody2D> ().AddRelativeForce (new Vector2 (0,10000));
		}
